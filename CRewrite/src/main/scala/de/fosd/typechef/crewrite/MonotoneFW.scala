...
abstract class MonotoneFWIdLab(env: ASTEnv, udm: UseDeclMap, fm: FeatureModel) extends MonotoneFW[Id](env, udm, fm) {
    // add annotation to elements of a Set[PGT]
    // this function is used to add feature expression information to set generated by gen and kill
    protected def addAnnotations(in: Set[PGT]): L = {
        var res = l
    ...
    def t2T(i: PGT) = Id(i.name + "_" + System.identityHashCode(i))

    def t2SetT(i: Id) = Set(addFreshT(i))
}
