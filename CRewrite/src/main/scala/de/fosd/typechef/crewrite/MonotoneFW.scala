...
abstract class MonotoneFW[T](val env: ASTEnv, val udm: UseDeclMap, val fm: FeatureModel) extends AttributionBase with IntraCFG {
    ...
    type PGT = T
    protected def l = Map[PGT, FeatureExpr]()
    ...
    private def createLFromSetT(s: L): L = {
...
abstract class MonotoneFWIdLab(env: ASTEnv, udm: UseDeclMap, fm: FeatureModel) extends MonotoneFW[(Id, Int)](env, udm, fm) {

    // add labels in the form of hashcodes
    protected def addLabels(s: Set[Id]) = {
        s.map(x => (x, System.identityHashCode(x)))
    }
    ...
    def t2T(i: (Id, Int)) = (Id(getFreshCtr + "_" + i._1.name), i._2)

    def t2SetT(i: (Id, Int)) = {
        var freshidset = Set[(Id, Int)]()

        if (udm.containsKey(i._1)) {
            for (vi <- udm.get(i._1)) {
                freshidset = freshidset.+(createFresh((vi, System.identityHashCode(i))))
            }
            freshidset
        } else {
            Set(addFreshT(i))
