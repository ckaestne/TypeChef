...
trait ConditionalControlFlow extends ASTNavigation with ConditionalNavigation {
    ...
    private def succHelper(source: AST, ctx: FeatureExpr, oldres: CCFGRes, fm: FeatureModel, env: ASTEnv): CCFGRes = {
        source match {
            ...
            case t@ReturnStatement(retExpr) => {
                findPriorASTElem[FunctionDef](t, env) match {
                    ...
                    case Some(f) => {
                        ...
                        if (retExpr.isDefined)
                            findMethodCalls(retExpr.get, env, oldres, ctx, res)
                        else res
            ...
            //      case t: DefaultStatement => getStmtSucc(t, ctx, oldres, fm, env) // no special handling necessary
    ...
    def findMethodCalls(t: AST, env: ASTEnv, oldres: CCFGRes, ctx: FeatureExpr, _res: CCFGRes): CCFGRes = {
        var res: CCFGRes = _res
    ...
    private def getExprSucc(exp: Expr, ctx: FeatureExpr, oldres: CCFGRes, fm: FeatureModel, env: ASTEnv): CCFGRes =
        findMethodCalls(exp, env, oldres, ctx, oldres) ++ {
            exp match {
                case c@CompoundStatementExpr(CompoundStatement(innerStatements)) => {
                    if (barrier.exists(e => e.eq(c))) {
            ...
            case _ => {
                ...
                surrounding_parent match {
                    ...
                    case t@WhileStatement(expr, _) => {
                        if (nested_ast_elem.eq(expr)) getStmtPred(t, ctx, oldres, fm, env)
    ...
    def getAllSucc(i: AST, fm: FeatureModel, env: ASTEnv): List[(AST, List[Opt[AST]])] = {
        ...
        while (!s.isEmpty) {
            ...
            if (!c.isInstanceOf[ExternalDef] || (c eq i)) //stay within one method
