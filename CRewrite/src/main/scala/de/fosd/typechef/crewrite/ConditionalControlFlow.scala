...
trait ConditionalControlFlow extends ASTNavigation with ConditionalNavigation {
  ...
  private def getCondStmtSucc(c: Conditional[Statement], ctx: FeatureExpr, oldres: CCFGRes, env: ASTEnv): CCFGRes = {
    c match {
      ...
      case One(c@CompoundStatement(l)) => getCompoundSucc(l, c, ctx, oldres, env)
      case One(s: Statement) => (env.featureExpr(s), s) :: oldres
    }
  }

  private def getCondStmtPred(p: AST, c: Conditional[_], ctx: FeatureExpr, oldres: CCFGRes, env: ASTEnv): CCFGRes = {
    c match {
      case Choice(_, thenBranch, elseBranch) =>
        getCondStmtPred(p, thenBranch, ctx, oldres, env) ++ getCondStmtPred(p, elseBranch, ctx, oldres, env)
      case One(c@CompoundStatement(l)) => getCompoundPred(l.reverse, c, ctx, oldres, env)
      case One(s: Statement) => (env.featureExpr(s), s) :: oldres
    }
  }
        ...
        surrounding_parent match {
          ...
          case t@IfStatement(condition, thenBranch, elifs, elseBranch) => {
            if (isPartOf(nested_ast_elem, condition)) getStmtPred(t, ctx, oldres, env)
              ...
              if (barrier.exists(x => curelem.eq(x))) { getCondExprPred(condition, ctx, oldres, env) }
                ...
                barrier ::= curelem
                val res = getStmtPred(curelem, ctx, oldres, env)
          ...
          case t@ElifStatement(condition, thenBranch) => {
            if (isPartOf(nested_ast_elem, condition)) {
              var res = getStmtPred(t, ctx, oldres, env)
              val predelifs = prevASTElems(t, env).tail
              val predelifsfexp = predelifs.map(env.featureExpr).fold(FeatureExprFactory.False)(_ or _)
  ...
  private def getCompoundPred(l: List[Opt[AST]], parent: AST, ctx: FeatureExpr, oldres: CCFGRes, env: ASTEnv): CCFGRes = {
    if (l.isEmpty) {
      if (predComplete(ctx, oldres)) oldres
        ...
        if (! barrier.exists(x => x.eq(parent))) followPred(parent, ctx, oldres, env)
        else oldres
      }
    } else {
      var curres = oldres
        ...
        x => {
          ...
          if (ctxx and ctx isContradiction()) { }
            ...
            if (isCFGInstruction(x.entry)) curres ::= (ctxx, x.entry)
              ...
              barrier ::= x.entry
              curres = predHelper(x.entry, ctx, curres, env)
              barrier = barrier.filterNot(e => e.eq(x.entry))
            }
