...
add(a,b){return a+b;}
mul(a){return 85*a;}
memshift(p)unsigned*p;{unsigned x;for(;1;){x=*p++>>16;if(x)return x;}}
ldw(xp)si*xp;{return xp[4];}
ldws_m(xp)si*xp;{si x;do{x=xp[3];xp+=3;}while(x);}
postinc_si(p)si*p;{si x;for(;1;){x=*p++;if(x)return x;}}
preinc_si(p)si*p;{si x;for(;1;){x=*++p;if(x)return x;}}
postinc_di(p)di*p;{di x;for(;1;){x=*p++;if(x)return x;}}
preinc_di(p)di*p;{di x;for(;1;){x=*++p;if(x)return x;}}
