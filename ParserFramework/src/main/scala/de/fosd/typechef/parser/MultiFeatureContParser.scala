...
//
//  type CpsParseResult[+T] = ParseResult[T] @cpsParam[ParseResult[T], CpsParser[T]]
...
//   */
//  sealed abstract class CpsNoSuccess(val msg: String, override val next: Input) extends ParseResult[Nothing] { // when we don't care about the difference between CpsFailure and CpsError
...
//    def append[U >: Nothing](a: => CpsParseResult[U]): ParseResult[U] = this
//  }
//
...
//  def alternativeParser[U](next: CpsParser[U], newTopFeatOpt: Option[FeatureExpr]) : CpsParser[Any] =
//	  CpsParser[Any]((in: Input) => /*: CpsParseResult[U] @cpsParam[CpsParseResult[U], CpsParser[U]] = {*/
//	  shift {
//	 	  (captCont: CpsParseResult[Any] => CpsParseResult[Any]) =>
...
//        (for(a <- this; b <- p) yield new ~(a,b)).named("~")
//    	/*this.flatMap((resThis: T) => CpsParser[~[T, U]] ({
//    	    (in: Input) => CpsSuccess(new ~(resThis, p(in.first)(in)))
//    	}))*/
//      //(for (a <- this; b <- p) yield new ~(a, b)) //.named("~")*/
//
