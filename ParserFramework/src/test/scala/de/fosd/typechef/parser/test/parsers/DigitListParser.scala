...
class DigitListParser extends ConditionalParserLib {
    type Elem = MyToken
    ...
    def digits: ConditionalParser[Conditional[AST]] =
        (digit ~ opt(digits) ^^! {
    ...
    def digit: ConditionalParser[Lit] = token("digit", ((x) => x.t == "1" | x.t == "2" | x.t == "3" | x.t == "4" | x.t == "5")) ^^ {
        t => Lit(t.text.toInt)
    }
