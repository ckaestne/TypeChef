...
class CharDigitParser extends ConditionalParserLib {
    type Elem = MyToken
    ...
    def char: ConditionalParser[AST] =
        token("char", ((x) => x.t == "a" | x.t == "b" | x.t == "c" | x.t == "d" | x.t == "e")) ^^ {
            (x: Elem) => Char(x.text)
        }

    def expr: ConditionalParser[Conditional[AST]] = (expr1 ~ opt(t("*") ~> expr) ^^! ({
        case ~(f, Some(e)) => One(Mul(f, e))
        case ~(f, None) => f
    })).map(ConditionalLib.combine(_))

    def expr1: ConditionalParser[Conditional[AST]] = (expr2 ~ opt(t("+") ~> expr) ^^! ({
        case ~(f, Some(e)) => One(Plus(f, e))
        case ~(f, None) => f
    })).map(ConditionalLib.combine(_))

    def expr2: ConditionalParser[Conditional[AST]] = t("(") ~> expr <~ t(")") | (digit.join)
