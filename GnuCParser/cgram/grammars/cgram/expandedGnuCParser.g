...
typeQualifierList :(typeQualifier | attributeDecl)+;
...
:( pointerGroup )?               
                ...
                | LPAREN (attributeDecl)? declName = declarator[false] RPAREN
                )
...
attributeDecl :"__attribute__"^ LPAREN LPAREN attributeList RPAREN RPAREN
...
strOptExprPair :(LBRACKET ID RBRACKET)? stringConst ( LPAREN expr RPAREN )?
        ;

primaryExpr :"__builtin_offsetof" LPAREN typeName COMMA offsetofMemberDesignator RPAREN
        |       ID
        |       Number
        |       charConst
        |       stringConst
...
offsetofMemberDesignator :ID (DOT ID)* ;
...
// inherited from grammar StdCParser
pointerGroup :( STAR ( typeQualifierList )? )+    { ## = #( #[NPointerGroup], ##); }
        ;
