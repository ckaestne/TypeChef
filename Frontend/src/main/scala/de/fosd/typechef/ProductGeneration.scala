...
object ProductGeneration extends EnforceTreeHelper {
   ...
   *
   */
  def buildConfigurations(family_ast: TranslationUnit, fm: FeatureModel, configSerializationDir: File): (String, List[Task]) = {
    var msg: String = ""
    var log: String = ""
  ...
  def createAndSaveConfigurations(ast: AST, fm: FeatureModel, opt: FrontendOptions) {
  ...
  def typecheckProducts(fm_scanner: FeatureModel, fm_ts: FeatureModel, ast: AST, opt: FrontendOptions) {
    ...
    val (configGenLog: String, typecheckingTasks: List[Pair[String, List[SimpleConfiguration]]]) =
      buildConfigurations(family_ast, fm_ts, configSerializationDir)
  ...
  def dataflowAnalysisProducts(fm_scanner: FeatureModel, fm_ts: FeatureModel, ast: AST, opt: FrontendOptions) {
    ...
    dataflowAnalysisConfigurations(tasks, family_ast, fm, family_ast, opt, startLog = configGenLog)
  }

  def dataflowAnalysisConfigurations(tasks: List[Task], family_ast: AST, fm: FeatureModel, ast: AST,
                                      opt: FrontendOptions, startLog: String = "") {
    val log: String = ""
    val thisFilePath = opt.getFile.substring(opt.getFile.lastIndexOf("linux-2.6.33.3"))
    println("starting product typechecking.")
  ...
  */
  def configurationCoverage(astRoot: TranslationUnit, fm: FeatureModel, features: List[SingleFeatureExpr],
    ...
    // TODO collectAnnotationNodes omits nested Opt and Choice nodes
  ...
  def loadConfigurationsFromHenardFiles(files: List[File], dimacsFile: File, features: List[SingleFeatureExpr], fm: FeatureModel): (List[SimpleConfiguration], String) = {
    ...
    (retList, "Unsat Configs:" + unsat_configs.mkString("{", ",", "}"))
  }
  ...
  def completeConfiguration_Inefficient(partialConfig: FeatureExpr, remainingFeatures: List[FeatureExpr], fm: FeatureModel, preferDisabledFeatures: Boolean = true): FeatureExpr = {
    ...
    val partConfigFeasible: Boolean = true
